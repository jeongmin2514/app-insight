...
4. 오늘의 액션 생성: 수집한 지표를 보고 `data/today_actions.json`을 **오늘 날짜로 전체 교체**한다.
   ...
   - **연속성**: 갱신 전에 기존 `today_actions.json`(어제 액션)·`data/events.json`(운영 로그)·`notes/` 최신 노트를 읽어라.
     어제와 똑같은 액션을 반복하지 말고 후속 단계로 진전시킬 것 (예: 어제 "기획 착수" → 오늘 "요건 조사", 지표 변화 대기 중이면 그렇게 명시).
     실행 여부를 모르는 액션은 "~ 이어서"로 이어가되 완료로 단정하지 말 것.
